...
class HandDetector():
    def __init__(self, mode=False, maxHands=2, detectionCon=0.5, trackCon=0.5) -> None:
        ...
        self.mp_hands = hands
        self.hands = hands.Hands(self.mode, self.maxHands, 1, self.detectionCon, self.trackCon)
    ...
    def __find_hands(self, img:cv2.Mat, draw=True):
        ...
        for hand_lms in results.multi_hand_landmarks: # type: ignore
            drawing_utils.draw_landmarks(img, hand_lms, hands.HAND_CONNECTIONS) # type: ignore
        return (results, True)
    ...
    def find_hands(self, img:cv2.Mat, hand_NO=0, draw=True) :
        ...
        if draw:
            cv2.rectangle(img, (xmin - 20, ymin - 20), (xmax + 20, ymax + 20), 2)
